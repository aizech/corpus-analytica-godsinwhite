...
# Page title
st.title(":material/photo_library: Generated Images Gallery")
...
def display_image_gallery(image_files):
    ...
    if not image_files:
        st.info(
            "No images have been generated yet. Try generating an image using the GPTImage1 agent!"
        )
        return
    ...
    for i, image_file in enumerate(image_files):
        ...
        with cols[col_idx]:
            try:
                ...
                with col1:
                    if st.button("  :material/search:  ", key=f"view_{i}"):
                        try:
                            ...
                            st.warning(
                                "Could not open in a new window. Displaying larger version here:"
                            )
                            st.image(
                                img,
                                caption=f"{filename} ({img.width}×{img.height} pixels)",
                                width="stretch",
                            )
                ...
                with col3:
                    if st.button(
                        ":material/delete:", key=f"delete_{i}", type="primary"
                    ):
                ...
                if st.session_state.get("delete_confirm") == i:
                    ...
                    with confirm_col1:
                        if st.button(
                            ":material/check_circle: Yes, delete it",
